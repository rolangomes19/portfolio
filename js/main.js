...
(() => {
  ...
  const STRINGS = {
    en: {
      ...
      "meta.title": "Rolan Gomes - UX Designer · Design Systems & AI-Directed Delivery",
      "meta.description": "Rolan Gomes is a UX Designer who builds design systems, then directs AI to turn them into working software. Design Systems, Code Connect, and WCAG 2.2 Accessibility. UAE-based, open to opportunities across Dubai, Abu Dhabi, and Sharjah.",

      "hero.title": "I design systems, then direct AI to build them into working software.",
      ...
      "highlight.poc.label": "AI-directed frontend POC",
      ...
      "work.poc.desc": "Using the Design System components, we directed and built WorkFeed, a functioning frontend POC of a task management feature. Achieved an 80% component match rate while converting Design to code.",
      ...
      "meta.poc.title": "Making Design Systems Legible to AI - Rolan Gomes",
      "meta.poc.description": "Case study: how the Blueprint Design System's token discipline and Code Connect mapping made an AI-directed frontend build possible, an 80% component match rate in a 7-day turnaround on the WorkFeed proof of concept.",
      "case.poc.summary": "A design-to-code experiment using the Blueprint Design System and WorkFeed, a task management proof of concept on the Hub platform. An LLM didn't build this frontend. I built the conditions under which it could, then directed it precisely: an 80% component match rate in a 7-day turnaround, with little to no manual fixing needed.",
      "case.poc.summary.problem": "<strong>Problem.</strong> Most teams were adopting AI by pointing it at whatever step already felt slow, treating it as a plug-in for the existing process instead of asking what AI actually needs from a team to produce something reliable.",
      "case.poc.summary.role": "<strong>Role.</strong> I owned the design-phase mechanism (Figma Make, prompt structure, guideline authoring), the token discipline that made the handoff work, and the review process that decided what shipped.",
      "case.poc.summary.impact": "<strong>Impact.</strong> 80% of WorkFeed's components converted correctly with little to no manual fixing. The 20% that didn't pointed to a specific, fixable cause: component granularity, not model capability.",
      "case.poc.summary.stack": "<strong>Stack.</strong> Figma Make · Figma Code Connect · Figma MCP server · GitHub Copilot (VS Code) · Claude · Style Dictionary",
      "case.poc.premise.p1": "AI can generate a design or a component fast. Whether it generates something usable fast depends almost entirely on what you give it to work with. A design system built for humans, consistent components, named variants, clear rules, turns out to be exactly what a model needs too. An inconsistent one doesn't get fixed by AI. It gets guessed at, faster.",
      "case.poc.premise.p2": "Most teams at the firm were approaching AI adoption by looking for a slow step and pointing a model at it. That's a reasonable starting instinct, but it treats AI as a plug-in for the existing process rather than asking a more basic question: what does AI actually need from us to produce something reliable? We asked the second question. Blueprint, the design system I'd been building, turned out to already have most of the answer, because the same discipline that makes a system good for a human developer, no raw values, clear component boundaries, documented intent, makes it legible to a model too. I hadn't built it with AI in mind. It became AI-ready as a byproduct of being built well.",
      "case.poc.changed.p1": "The rule that mattered most: nothing in a finished design ships with a raw color or spacing value. Every choice ties back to a token, component-level first, semantic as a fallback, nothing hardcoded. That discipline was already how I ran Blueprint. What changed is that it turned this system into something a coding assistant could actually read, instead of something it had to guess at from a picture.",
      "case.poc.changed.p2": "Behind that rule sits a small set of structured files: separate files for the raw values, the semantic layer that gives them meaning, and the component tokens built on top of those. A markdown file explains how the three layers connect. Each component has its own file describing when to use which variant, written the way you'd brief a new engineer joining the team. Style Dictionary generates all of this from the source definitions, so none of it is duplicate, hand-maintained work.",
      "case.poc.changed.p3": "On the code side, Figma's Code Connect maps each design component to its real counterpart in the codebase. The Figma MCP server exposes that mapping, along with the token values and component structure, as information a coding assistant can read directly. We ran this through GitHub Copilot inside VS Code, with Claude writing the actual code. Worth being precise about what that bridge does: it doesn't generate anything itself. It hands the model a well-organized starting point instead of a screenshot to guess from. The generation still comes from the model.",
      "case.poc.workflow.intro": "Four stages, each with a different job for AI and a different job for a person.",
      "case.poc.origin": "This system also grew out of earlier, separate experimentation. Before design-to-code became the focus, we tested whether AI could help even earlier, in live client work, generating rough prototypes during strategy conversations instead of spending hours on wireframes between meetings. It worked for what it was: decisions moved faster, exploration got cheaper. But the output stayed rough, not something you'd hand a client as finished work, and a designer still made every real call. That early test is part of why we trusted the bigger bet on design-to-code. It's not the main story here. WorkFeed is.",
      "case.poc.workfeed.intro": "WorkFeed, a task management proof of concept inside the Hub platform, is where this pipeline actually ran, under a real deadline, in front of someone who could greenlight it beyond a prototype.",
      "case.poc.convergence": "Two people, coming at the same problem from opposite ends, landing on the same root cause. That convergence is better evidence the underlying idea holds than the 80% number is by itself. It also gave Blueprint's existing component-review process a clear new question to ask of every new component: is this granular enough for a model to map cleanly, not just clear enough for a designer to use.",
      "case.poc.worked": "Design exploration got faster, because AI could produce several directions in parallel instead of one at a time. Design-to-code got more predictable, because the model was working from structured tokens and mappings instead of inferring from a screenshot. And the failure pattern taught us something specific and useful: component granularity, not model capability, was the actual ceiling.",
      "case.poc.cost": "I'm not going to give you a time-saved or cost-saved number. Not because it wasn't worth asking, but because a fixed figure in this space goes stale fast, both what these models cost and what they can do are moving quickly enough that a specific number from late 2025 won't mean much by the time anyone reads this. The more useful question, and the one we were actually trying to answer, isn't what this saved once. It's whether a team has a real way to keep judging if the investment is still worth making as the ground shifts under it. That's a harder answer to give than a percentage. It's also the honest one.",
      "case.poc.leaves": "Blueprint already had a working process for deciding what gets built into it: who reviews a request, how it gets scoped, what happens when the answer is no. WorkFeed adds a clear argument for extending that process, scoping every new component for how well a model can parse it, not just how well a person can use it, at design time, before it ever reaches a code-gen pipeline. I wasn't at the org long enough to see whether that became a formal part of the process, so I can't claim that it did. But the case for it is sitting right there in the 80/20 result.",
      "case.poc.carry.p1": "We built this against a real deadline, in front of someone who could take it straight to production. That mattered more than it sounds like it should. If the proof of concept worked, there was a real risk it would get pulled out of our hands before it was actually ready to be a shippable feature. So the goal on the design side was never to make something that looked impressive. It was to make sure that if this did take off, what we handed over could still function as a real first version, not a demo that fell apart the moment someone tried to build on it.",
      "case.poc.carry.p2": "That pressure shaped the harder calls more than anything else. When two directions don't clearly differ in impact, and you don't have enough information yet to know which one's right, you're working from your best judgment, not certainty. That's true of design generally, not a flaw specific to this project. We made real decisions under that uncertainty, and we changed direction quickly more than once when something new came up. I don't see that as a weakness in the process. I see it as what the process is for.",
      "case.poc.carry.p3": "The idea that actually holds this whole project together didn't arrive as a plan. It came together in pieces, watching what other design system teams were doing, a deadline that forced real scope discipline, and a reframing I picked up secondhand that happened to land at the right time. Every piece, arrived at separately, kept pointing at the same underlying fact: a design system built with real discipline doesn't just serve designers and engineers anymore. It's also what makes AI worth using at all. Design systems aren't becoming less important because AI can generate interfaces. They're becoming the thing that decides whether AI-generated interfaces are any good.",
      "case.poc.quote": "At a UX conference, I heard Kate Moran of Nielsen Norman Group describe working with AI the way you'd work with a sharp, well-read intern: someone you guide closely rather than hand a decision to, who gets better the more context you give them, and whose real edge over a human intern is speed and breadth, not judgment. She's written the idea up formally in <a href=\"https://www.nngroup.com/articles/ai-intern/\" target=\"_blank\" rel=\"noopener noreferrer\">\"Your AI UX Intern\"</a>. It reframed the question for me from \"can I trust this\" to \"how do I set this up to do good work,\" and that's the posture behind everything above: structure first, human judgment at the end, always.",
      ...
      "case.hub.summary.problem": "<strong>Problem.</strong> A Power Pages data-model migration was required to unlock Bootstrap 5, which forced a full front-end rebuild and surfaced 35 WCAG Critical/Serious violations, mismatched Azure AD B2C flows, and three uncoordinated loading spinners.",
      "case.hub.summary.role": "<strong>Role.</strong> Design for the visual system upgrade; front-end development for the Bootstrap rebuild, the 11 Azure AD B2C flows, and the spinner consolidation.",
      "case.hub.summary.impact": "<strong>Impact.</strong> 35 → 0 WCAG violations remediated. 14 of roughly 20 page-level CSS files consolidated into one token-based stylesheet. All five efforts shipped to production.",
      "case.hub.summary.stack": "<strong>Stack.</strong> Bootstrap 3→5 · Power Pages · Dynamics 365 · Azure AD B2C · Deque Axe",
      ...
      "case.speery.summary.problem": "<strong>Problem.</strong> An AI-generated prototype was functionally complete but structurally empty: colors carried no consistent meaning, no hierarchy, nothing signaling trustworthiness to an enterprise buyer.",
      "case.speery.summary.role": "<strong>Role.</strong> Solo, design to handoff. Figma only; build handed to the client's engineering team.",
      "case.speery.summary.impact": "<strong>Impact.</strong> A full information-architecture rebuild, a three-axis semantic tagging system, and a reusable navigation pattern, delivered in two weeks, start to handoff.",
      "case.speery.summary.stack": "<strong>Stack.</strong> Figma · Smart Animate prototyping · semantic color/tagging system",
      ...
      "case.speery.readtwice.p2": "The brief asked for a system specific enough to be read by an AI downstream, not just a person, the same instinct behind Blueprint's token structure being built for both a human developer and an LLM from day one. See <a href=\"blueprint-design-system.html\">Blueprint Design System</a>.",
      ...
      "case.bds.summary": "The Hub platform had five squads shipping the same button five different ways, and no bridge between Figma and code besides screenshots and goodwill. I led design on the system that fixed both, working alongside a UX Lead Architect and a frontend engineer: an Atomic Design component hierarchy sitting on top of a three-tier OKLCH token architecture, and the Code Connect links that turned out to be exactly what an LLM needed too, accurate enough to hit an 80% design-to-code conversion rate in a working proof of concept.",
      "case.bds.summary.problem": "<strong>Problem.</strong> Five squads on the Hub platform were shipping the same button five different ways, with no bridge between Figma and code beyond screenshots.",
      "case.bds.summary.role": "<strong>Role.</strong> Design lead, foundational architecture through documentation, alongside a UX Lead Architect (final sign-off) and a frontend engineer (Phase 2, Code Connect).",
      "case.bds.summary.impact": "<strong>Impact.</strong> An 80% accurate LLM component-conversion rate in a design-to-code proof of concept. Phase 1 complete, Phase 2 in development, not yet live in production.",
      "case.bds.summary.stack": "<strong>Stack.</strong> Figma · OKLCH tokens · Atomic Design · Code Connect · React · Tailwind v4",
      ...
      "case.bds.reflection.p1": "Submit for review earlier than feels ready. Phase 1 sat finished but unsubmitted longer than it needed to, and a solid system loses momentum sitting unreviewed faster than extra polish ever adds to it.",
    },
    "en-simple": {
      "meta.title": "Rolan Gomes - UX Designer, Design Systems and AI",
      "meta.description": "Rolan Gomes is a UX Designer. He builds design systems, then directs AI to turn them into working software. He works on Design Systems, Code Connect, and WCAG 2.2 Accessibility. He is based in the UAE, and open to opportunities in Dubai, Abu Dhabi, and Sharjah.",

      "hero.title": "I design systems. Then I direct AI to build them into working software.",
      "hero.sub": "I am a UX Designer. I can also build software, like an engineer. I built a Design System from the ground up, including the token architecture and the Code Connect links, for a top-20 US accounting and consulting firm. Then I directed an AI tool to turn the system into a working frontend. The AI matched 80% of the components correctly. This took 7 days. I am now looking for new opportunities across the UAE.",
      ...
      "work.poc.desc": "We used the Design System components. We directed an AI tool to build WorkFeed, a working frontend proof-of-concept. The feature was task management. The AI matched 80% of the components correctly when it converted the design to code.",
      ...
      "meta.poc.title": "Making Design Systems Legible to AI - Rolan Gomes",
      "meta.poc.description": "Case study: I used the Blueprint Design System's token rules and Code Connect links to let an AI-directed tool build a frontend. The result was an 80% component match rate in 7 days, on the WorkFeed proof of concept.",
      "case.poc.summary": "This is a design-to-code experiment. It uses the Blueprint Design System and WorkFeed, a task management proof of concept on the Hub platform. An AI tool did not build this frontend by itself. I built the conditions that let it work well, then directed it closely. The result was an 80% component match rate in 7 days, with little manual fixing needed.",
      "case.poc.summary.problem": "<strong>Problem.</strong> Most teams adopted AI by pointing it at a slow step in their existing process. They did not ask what AI actually needs from a team to give reliable results.",
      "case.poc.summary.role": "<strong>Role.</strong> I owned the design-phase method (Figma Make, prompt structure, and guideline writing), the token discipline that made the handoff work, and the review process that decided what shipped.",
      "case.poc.summary.impact": "<strong>Impact.</strong> 80% of WorkFeed's components converted correctly, with little to no manual fixing. The other 20% pointed to one fixable cause: component granularity, not model capability.",
      "case.poc.summary.stack": "<strong>Stack.</strong> Figma Make · Figma Code Connect · Figma MCP server · GitHub Copilot (VS Code) · Claude · Style Dictionary",
      "case.poc.premise.p1": "AI can generate a design or a component fast. But whether that output is usable depends on what you give it to work with. A design system built well for humans, with consistent components, named variants, and clear rules, turns out to work well for a model too. An inconsistent design system does not get fixed by AI. It gets guessed at, faster.",
      "case.poc.premise.p2": "Most teams at the firm approached AI adoption by finding a slow step and pointing a model at it. That is a reasonable first instinct. But it treats AI as a plug-in for the existing process. It does not ask a more basic question: what does AI actually need from us to give reliable results? We asked that second question. Blueprint, the design system I had been building, already had most of the answer. The same discipline that makes a system good for a human developer, no raw values, clear component boundaries, documented intent, also makes it easy for a model to read. I did not build it with AI in mind. It became AI-ready as a side effect of being built well.",
      "case.poc.changed.p1": "The most important rule: no finished design ships with a raw color or spacing value. Every choice ties back to a token, first at the component level, then the semantic level as a fallback. Nothing is hardcoded. This was already how I ran Blueprint. What changed is that this discipline turned the system into something a coding assistant could actually read, instead of something it had to guess at from a picture.",
      "case.poc.changed.p2": "Behind that rule sits a small set of structured files. Separate files hold the raw values, the semantic layer that gives them meaning, and the component tokens built on top. A markdown file explains how the three layers connect. Each component has its own file that describes when to use which variant, written the way you would brief a new engineer joining the team. Style Dictionary generates all of this from the source files, so nothing is duplicated or kept up to date by hand.",
      "case.poc.changed.p3": "On the code side, Figma's Code Connect maps each design component to its real counterpart in the codebase. The Figma MCP server shares that mapping, plus the token values and component structure, as information a coding assistant can read directly. We ran this through GitHub Copilot inside VS Code, with Claude writing the actual code. To be precise about what that bridge does: it does not generate anything itself. It gives the model a well-organized starting point instead of a screenshot to guess from. The model still does the generating.",
      "case.poc.workflow.intro": "There are four stages. Each one gives AI a different job, and gives a person a different job.",
      "case.poc.origin": "This system also grew out of earlier, separate experiments. Before design-to-code became the focus, we tested whether AI could help even earlier, in live client meetings. It generated rough prototypes during strategy conversations, instead of us spending hours on wireframes between meetings. It worked for what it was: decisions moved faster, and exploration got cheaper. But the output stayed rough. It was not something you would hand a client as finished work, and a designer still made every real call. That early test is part of why we trusted the bigger bet on design-to-code. It is not the main story here. WorkFeed is.",
      "case.poc.workfeed.intro": "WorkFeed is a task management proof of concept inside the Hub platform. It is where this pipeline actually ran, under a real deadline, in front of someone who could take it beyond a prototype.",
      "case.poc.convergence": "Two people came at the same problem from opposite ends and landed on the same root cause. That match is better evidence that the underlying idea holds than the 80% number is by itself. It also gave Blueprint's existing component-review process a new question to ask of every new component: is this granular enough for a model to map cleanly, not just clear enough for a designer to use?",
      "case.poc.worked": "Design exploration got faster, because AI could produce several directions at once instead of one at a time. Design-to-code got more predictable, because the model worked from structured tokens and mappings instead of guessing from a screenshot. And the failure pattern taught us something specific and useful: component granularity, not model capability, was the real limit.",
      "case.poc.cost": "I will not give you a time-saved or cost-saved number. It is not because that number was not worth finding. It is because a fixed figure in this space goes stale fast. Both what these models cost, and what they can do, are changing quickly. A specific number from late 2025 will not mean much by the time you read this. The more useful question, and the one we actually tried to answer, is not what this saved once. It is whether a team has a real way to keep checking if the investment is still worth making as things change. That is a harder answer to give than a percentage. It is also the honest one.",
      "case.poc.leaves": "Blueprint already had a working process for deciding what gets built into it: who reviews a request, how it gets scoped, and what happens when the answer is no. WorkFeed makes a clear case for extending that process. Every new component should be scoped for how well a model can read it, not only for how well a person can use it, at design time, before it reaches a code-generation pipeline. I did not stay at the org long enough to see if that became a formal part of the process, so I cannot claim that it did. But the case for it is right there in the 80/20 result.",
      "case.poc.carry.p1": "We built this against a real deadline, in front of someone who could take it straight to production. That mattered more than it sounds like it should. If the proof of concept worked, there was a real risk it would get pulled out of our hands before it was ready to be a shippable feature. So the goal on the design side was never to look impressive. It was to make sure that, if this did take off, what we handed over could still work as a real first version, not a demo that fell apart the moment someone tried to build on it.",
      "case.poc.carry.p2": "That pressure shaped the harder calls more than anything else. When two directions do not clearly differ in impact, and you do not yet have enough information to know which one is right, you work from your best judgment, not certainty. That is true of design in general, not a flaw specific to this project. We made real decisions under that uncertainty, and we changed direction quickly more than once when something new came up. I do not see that as a weakness in the process. I see it as what the process is for.",
      "case.poc.carry.p3": "The idea that holds this whole project together did not arrive as a plan. It came together in pieces: watching what other design system teams were doing, a deadline that forced real scope discipline, and a reframing I picked up secondhand that landed at the right time. Every piece, found separately, pointed at the same fact: a design system built with real discipline does not just serve designers and engineers anymore. It is also what makes AI worth using at all. Design systems are not becoming less important because AI can generate interfaces. They are becoming the thing that decides whether AI-generated interfaces are any good.",
      "case.poc.quote": "At a UX conference, I heard Kate Moran of Nielsen Norman Group describe working with AI like working with a sharp, well-read intern. You guide this intern closely, rather than handing it a decision to make alone. It gets better the more context you give it. Its real edge over a human intern is speed and breadth, not judgment. She wrote the idea up formally in <a href=\"https://www.nngroup.com/articles/ai-intern/\" target=\"_blank\" rel=\"noopener noreferrer\">\"Your AI UX Intern\"</a>. It changed my question from \"can I trust this\" to \"how do I set this up to do good work.\" That is the posture behind everything above: structure first, human judgment at the end, always.",
      ...
      "case.hub.summary.problem": "<strong>Problem.</strong> A Power Pages data-model migration was needed to unlock Bootstrap 5. This forced a full front-end rebuild. The rebuild uncovered 35 WCAG Critical and Serious violations, Azure AD B2C flows that did not match the platform, and three loading spinners with no coordination.",
      "case.hub.summary.role": "<strong>Role.</strong> I designed the visual system upgrade. I also built the front end for the Bootstrap rebuild, the 11 Azure AD B2C flows, and the spinner fix.",
      "case.hub.summary.impact": "<strong>Impact.</strong> I fixed 35 WCAG violations, down to zero. I combined 14 of about 20 page-level CSS files into one token-based stylesheet. All five parts shipped to production.",
      "case.hub.summary.stack": "<strong>Stack.</strong> Bootstrap 3 to 5 · Power Pages · Dynamics 365 · Azure AD B2C · Deque Axe",
      ...
      "case.speery.summary.problem": "<strong>Problem.</strong> An AI tool built the first version. It worked, but it had no real structure. The colors did not mean anything. There was no clear hierarchy. Nothing signaled that an enterprise buyer could trust the interface.",
      "case.speery.summary.role": "<strong>Role.</strong> I worked alone, from design to handoff. I used Figma only. The client's engineering team built the final product.",
      "case.speery.summary.impact": "<strong>Impact.</strong> I rebuilt the information architecture. I built a three-axis tagging system. I created a reusable navigation pattern. This took two weeks, start to handoff.",
      "case.speery.summary.stack": "<strong>Stack.</strong> Figma · Smart Animate prototyping · semantic color and tagging system",
      ...
      "case.bds.summary": "The Hub platform had five teams. Each team built the same button in a different way. There was no link between Figma and code, only screenshots. I led the design work on the system that fixed this. I worked with a UX Lead Architect and a frontend engineer. We built an Atomic Design component hierarchy. It sits on a three-tier OKLCH token system. We also built Code Connect links. These links helped an LLM write code too, accurate enough to hit an 80% conversion rate in a working proof of concept.",
      "case.bds.summary.problem": "<strong>Problem.</strong> Five teams on the Hub platform built the same button five different ways. There was no link between Figma and code, except screenshots.",
      "case.bds.summary.role": "<strong>Role.</strong> I led the design work. This covered the foundational architecture and the documentation. I worked with a UX Lead Architect, who gave final sign-off, and a frontend engineer, who joined for Phase 2 and Code Connect.",
      "case.bds.summary.impact": "<strong>Impact.</strong> The AI matched 80% of the components correctly in a design-to-code test. Phase 1 is complete. Phase 2 is in development. It is not yet live in production.",
      "case.bds.summary.stack": "<strong>Stack.</strong> Figma · OKLCH tokens · Atomic Design · Code Connect · React · Tailwind v4",
    ...
    ar: {
      ...
      "meta.title": "رولان غوميس - مصمم UX، أنظمة التصميم والذكاء الاصطناعي",
      "meta.description": "رولان غوميس مصمم UX. يصمم أنظمة التصميم، ثم يوجّه الذكاء الاصطناعي لتحويلها إلى برمجيات فعلية عاملة. يعمل على أنظمة التصميم، وCode Connect، وإمكانية الوصول وفق معيار WCAG 2.2. يقيم في الإمارات العربية المتحدة، ومنفتح على فرص عمل في دبي وأبوظبي والشارقة.",

      "hero.title": "أصمم الأنظمة، ثم أوجّه الذكاء الاصطناعي لبنائها كبرمجيات فعلية عاملة.",
      "hero.sub": "أنا مصمم UX أستطيع أيضًا بناء البرمجيات، مثل المهندس. بنيت نظام تصميم بالكامل، من بنية الرموز (token architecture) إلى روابط Code Connect، لشركة أمريكية كبرى في المحاسبة والاستشارات ضمن أفضل ٢٠ شركة. ثم وجّهت أداة ذكاء اصطناعي لتحويل النظام إلى واجهة أمامية فعلية، بدقة تطابق ٨٠٪ خلال ٧ أيام. أبحث الآن عن فرص جديدة في جميع أنحاء الإمارات العربية المتحدة.",
      ...
      "highlight.poc.label": "نموذج أولي لواجهة أمامية موجَّه بالذكاء الاصطناعي",
      ...
      "work.poc.desc": "استخدمنا مكوّنات نظام التصميم. وجّهنا أداة ذكاء اصطناعي لبناء WorkFeed، نموذج أولي فعلي لواجهة أمامية. الميزة كانت إدارة المهام. طابقت الأداة ٨٠٪ من المكوّنات بدقة عند تحويل التصميم إلى كود.",
      ...
      "meta.poc.title": "جعل أنظمة التصميم مفهومة للذكاء الاصطناعي - رولان غوميس",
      "meta.poc.description": "دراسة حالة: كيف جعل انضباط الرموز التصميمية في نظام Blueprint Design System وربط Code Connect بناء واجهة أمامية موجّهة بالذكاء الاصطناعي أمرًا ممكنًا، بنسبة تطابق ٨٠٪ للمكوّنات خلال ٧ أيام في نموذج WorkFeed الأولي.",
      "case.poc.summary": "تجربة لتحويل التصميم إلى كود باستخدام نظام Blueprint Design System ونموذج WorkFeed الأولي لإدارة المهام على منصة Hub. لم يبنِ نموذج الذكاء الاصطناعي هذه الواجهة الأمامية بمفرده. أنا من بنى الشروط التي مكّنته من ذلك، ثم وجّهته بدقة: نسبة تطابق ٨٠٪ للمكوّنات خلال ٧ أيام، مع الحاجة لإصلاح يدوي محدود جدًا أو معدوم.",
      "case.poc.summary.problem": "<strong>المشكلة.</strong> تبنّت معظم الفرق الذكاء الاصطناعي بتوجيهه نحو خطوة بطيئة في العملية الحالية، دون أن تسأل عمّا يحتاجه الذكاء الاصطناعي فعليًا من الفريق لتقديم نتائج موثوقة.",
      "case.poc.summary.role": "<strong>الدور.</strong> قدت آلية مرحلة التصميم (Figma Make، وبنية التوجيهات، وكتابة الإرشادات)، وانضباط الرموز التصميمية الذي جعل التسليم ناجحًا، وعملية المراجعة التي قررت ما يُطلق.",
      "case.poc.summary.impact": "<strong>الأثر.</strong> طابقت ٨٠٪ من مكوّنات WorkFeed بشكل صحيح مع حاجة محدودة جدًا للإصلاح اليدوي. أشارت نسبة الـ٢٠٪ المتبقية إلى سبب محدد وقابل للإصلاح: حجم تفصيل المكوّن، لا قدرة النموذج.",
      "case.poc.summary.stack": "<strong>الأدوات.</strong> Figma Make · Figma Code Connect · Figma MCP Server · GitHub Copilot (VS Code) · Claude · Style Dictionary",
      "case.poc.premise.p1": "يستطيع الذكاء الاصطناعي توليد تصميم أو مكوّن بسرعة. لكن ما إذا كان الناتج قابلاً للاستخدام يعتمد بشكل شبه كامل على ما تمنحه للعمل به. نظام تصميم مبني جيدًا للبشر، بمكوّنات متسقة، وأنماط مسمّاة، وقواعد واضحة، يتضح أنه بالضبط ما يحتاجه النموذج أيضًا. النظام غير المتسق لا يُصلحه الذكاء الاصطناعي. بل يُخمَّن بشكل أسرع.",
      "case.poc.premise.p2": "تبنّت معظم الفرق في الشركة الذكاء الاصطناعي بالبحث عن خطوة بطيئة وتوجيه نموذج نحوها. هذه غريزة أولى معقولة، لكنها تعامل الذكاء الاصطناعي كإضافة للعملية الحالية بدلًا من طرح سؤال أساسي: ماذا يحتاج الذكاء الاصطناعي فعليًا منّا لتقديم نتائج موثوقة؟ طرحنا السؤال الثاني. تبيّن أن Blueprint، نظام التصميم الذي كنت أبنيه، يملك بالفعل معظم الإجابة، لأن الانضباط نفسه الذي يجعل النظام جيدًا لمطوّر بشري، دون قيم خام، وحدود واضحة للمكوّنات، ونية موثقة، يجعله مفهومًا لنموذج أيضًا. لم أبنِه بذهن الذكاء الاصطناعي. أصبح جاهزًا للذكاء الاصطناعي كنتيجة جانبية لكونه مبنيًا جيدًا.",
      "case.poc.changed.p1": "القاعدة الأهم: لا يُطلق أي تصميم نهائي بقيمة لون أو تباعد خام. كل خيار يعود إلى رمز تصميمي، على مستوى المكوّن أولًا، ثم الدلالي كبديل، دون أي قيمة مُثبّتة يدويًا. كان هذا الانضباط أصلًا الطريقة التي أدرت بها Blueprint. ما تغيّر هو أن هذا حوّل النظام إلى شيء يستطيع مساعد الترميز قراءته فعليًا، بدلًا من تخمينه من صورة.",
      "case.poc.changed.p2": "خلف هذه القاعدة مجموعة صغيرة من الملفات المنظَّمة: ملفات منفصلة للقيم الخام، والطبقة الدلالية التي تمنحها معنى، ورموز المكوّنات المبنية فوقها. يشرح ملف Markdown كيفية ارتباط الطبقات الثلاث. لكل مكوّن ملفه الخاص الذي يصف متى يُستخدم كل نمط، مكتوب بالطريقة نفسها التي تُوجَز بها لمهندس جديد ينضم للفريق. يولّد Style Dictionary كل هذا من التعريفات المصدرية، فلا شيء منه عمل مكرر يُصان يدويًا.",
      "case.poc.changed.p3": "من جهة الكود، يربط Code Connect في Figma كل مكوّن تصميمي بنظيره الفعلي في قاعدة الكود. يتيح خادم Figma MCP هذا الربط، إلى جانب قيم الرموز التصميمية وبنية المكوّنات، كمعلومات يستطيع مساعد الترميز قراءتها مباشرة. شغّلنا هذا عبر GitHub Copilot داخل VS Code، مع كتابة Claude للكود الفعلي. تجدر الدقة هنا حول ما يفعله هذا الجسر: هو لا يولّد شيئًا بنفسه. إنه يمنح النموذج نقطة انطلاق منظّمة جيدًا بدلًا من لقطة شاشة يُخمّن منها. التوليد لا يزال يأتي من النموذج.",
      "case.poc.workflow.intro": "أربع مراحل، لكل منها دور مختلف للذكاء الاصطناعي ودور مختلف للشخص.",
      "case.poc.origin": "نشأ هذا النظام أيضًا من تجارب سابقة ومنفصلة. قبل أن يصبح تحويل التصميم إلى كود هو التركيز، اختبرنا ما إذا كان الذكاء الاصطناعي يستطيع المساعدة في وقت أبكر، في عمل حقيقي مع عملاء، بتوليد نماذج أولية تقريبية أثناء محادثات الاستراتيجية بدلًا من قضاء ساعات في الرسومات التخطيطية بين الاجتماعات. نجح ذلك في حدود ما كان عليه: تسارعت القرارات، وأصبح الاستكشاف أرخص. لكن الناتج بقي تقريبيًا، ليس شيئًا تسلّمه لعميل كعمل نهائي، وظل المصمم من يتخذ كل قرار حقيقي. هذا الاختبار المبكر جزء من سبب ثقتنا بالرهان الأكبر على تحويل التصميم إلى كود. إنه ليس القصة الرئيسية هنا. WorkFeed هو القصة.",
      "case.poc.workfeed.intro": "WorkFeed، نموذج أولي لإدارة المهام داخل منصة Hub، هو المكان الذي عمل فيه هذا المسار فعليًا، تحت موعد نهائي حقيقي، أمام شخص يستطيع الموافقة عليه لتجاوز مرحلة النموذج الأولي.",
      "case.poc.convergence": "شخصان توصّلا إلى نفس السبب الجذري من طرفين مختلفين تمامًا للمشكلة. هذا التقارب دليل أقوى على صحة الفكرة الأساسية من رقم الـ٨٠٪ وحده. كما منح عملية مراجعة المكوّنات القائمة في Blueprint سؤالًا جديدًا واضحًا يُطرح على كل مكوّن جديد: هل هذا مفصّل بما يكفي ليُخطَّط بوضوح لنموذج، لا فقط واضح بما يكفي ليستخدمه مصمم؟",
      "case.poc.worked": "أصبح استكشاف التصميم أسرع، لأن الذكاء الاصطناعي استطاع إنتاج عدة اتجاهات في آن واحد بدلًا من واحد تلو الآخر. أصبح تحويل التصميم إلى كود أكثر قابلية للتنبؤ، لأن النموذج كان يعمل من رموز تصميمية وربط منظَّمين بدلًا من الاستنتاج من لقطة شاشة. وعلّمنا نمط الفشل شيئًا محددًا ومفيدًا: حجم تفصيل المكوّن، لا قدرة النموذج، كان السقف الحقيقي.",
      "case.poc.cost": "لن أعطيك رقمًا للوقت أو التكلفة الموفَّرة. ليس لأن السؤال لم يكن يستحق الطرح، بل لأن أي رقم ثابت في هذا المجال يتقادم بسرعة. كلٌّ من تكلفة هذه النماذج وقدراتها يتغيّر بسرعة كافية لجعل رقم من أواخر ٢٠٢٥ عديم المعنى تقريبًا بحلول وقت قراءة أحد لهذا. السؤال الأكثر فائدة، وهو ما كنّا نحاول فعليًا الإجابة عنه، ليس ما وفّره هذا مرة واحدة. بل ما إذا كان لدى الفريق طريقة حقيقية لمواصلة الحكم على ما إذا كان الاستثمار لا يزال يستحق مع تغيّر الأرض تحته. هذه إجابة أصعب من تقديم نسبة مئوية. وهي أيضًا الإجابة الصادقة.",
      "case.poc.leaves": "كان لدى Blueprint بالفعل عملية عاملة لتقرير ما يُبنى فيه: من يراجع الطلب، وكيف يُحدَّد نطاقه، وما يحدث حين تكون الإجابة لا. يضيف WorkFeed حجة واضحة لتوسيع تلك العملية، بحيث يُحدَّد نطاق كل مكوّن جديد وفق مدى قابليته للقراءة من نموذج، لا فقط قابليته للاستخدام من شخص، في وقت التصميم، قبل وصوله إلى مسار توليد الكود. لم أبقَ في المؤسسة طويلًا بما يكفي لأرى ما إذا أصبح هذا جزءًا رسميًا من العملية، فلا أستطيع الادعاء بأن ذلك حدث. لكن الحجة قائمة تمامًا في نتيجة الـ٨٠/٢٠.",
      "case.poc.carry.p1": "بنينا هذا في مواجهة موعد نهائي حقيقي، أمام شخص يستطيع أخذه مباشرة إلى الإنتاج. كان لهذا وزن أكبر مما يبدو عليه. لو نجح النموذج الأولي، كان هناك خطر حقيقي بأن يُسحب من أيدينا قبل أن يصبح فعليًا جاهزًا ليكون ميزة قابلة للإطلاق. لذلك لم يكن الهدف على جانب التصميم أبدًا أن يبدو مبهرًا. كان الهدف التأكد من أنه، إن نجح هذا فعلًا، سيبقى ما سلّمناه قادرًا على العمل كنسخة أولى حقيقية، لا عرضًا توضيحيًا ينهار بمجرد أن يحاول أحد البناء عليه.",
      "case.poc.carry.p2": "شكّل ذلك الضغط القرارات الصعبة أكثر من أي شيء آخر. حين لا يختلف اتجاهان بوضوح في الأثر، ولا تملك بعد معلومات كافية لمعرفة أيهما صحيح، تعمل من حكمك الأفضل، لا من يقين. هذا صحيح في التصميم عمومًا، وليس عيبًا خاصًا بهذا المشروع. اتخذنا قرارات حقيقية في ظل ذلك الغموض، وغيّرنا الاتجاه بسرعة أكثر من مرة حين ظهر شيء جديد. لا أرى في ذلك ضعفًا في العملية. أراه بالضبط ما وُجدت العملية من أجله.",
      "case.poc.carry.p3": "الفكرة التي تُمسك هذا المشروع بأكمله لم تصل كخطة. تشكّلت على مراحل: مراقبة ما كانت تفعله فرق أنظمة تصميم أخرى، وموعد نهائي فرض انضباطًا حقيقيًا في تحديد النطاق، وإعادة صياغة سمعتها من طرف آخر ووصلتني في الوقت المناسب. كل قطعة، وُجدت بشكل منفصل، أشارت إلى الحقيقة نفسها: نظام تصميم مبني بانضباط حقيقي لم يعد يخدم المصممين والمهندسين فقط. إنه أيضًا ما يجعل الذكاء الاصطناعي يستحق استخدامه أصلًا. أنظمة التصميم لا تصبح أقل أهمية لأن الذكاء الاصطناعي يستطيع توليد واجهات. إنها تصبح الشيء الذي يقرر ما إذا كانت الواجهات المولَّدة بالذكاء الاصطناعي جيدة أصلًا.",
      "case.poc.quote": "في مؤتمر لتجربة المستخدم، سمعت Kate Moran من Nielsen Norman Group تصف العمل مع الذكاء الاصطناعي كالعمل مع متدرب حاد الذكاء وواسع الاطلاع: توجّهه عن قرب بدلًا من تسليمه قرارًا كاملًا، ويتحسّن كلما منحته سياقًا أكثر، وميزته الحقيقية على المتدرب البشري هي السرعة والاتساع، لا الحكم. كتبت الفكرة رسميًا في <a href=\"https://www.nngroup.com/articles/ai-intern/\" target=\"_blank\" rel=\"noopener noreferrer\">\"Your AI UX Intern\"</a>. غيّرت هذه الفكرة سؤالي من \"هل أثق بهذا\" إلى \"كيف أُهيّئ هذا ليقوم بعمل جيد\"، وهذا هو الموقف خلف كل ما سبق: البنية أولًا، وحكم الإنسان في النهاية، دائمًا.",
      "case.bds.summary": "كانت منصة Hub تضم خمسة فرق. كل فريق بنى نفس الزر بطريقة مختلفة. لم يكن هناك رابط بين Figma والكود، سوى لقطات الشاشة. قدت العمل التصميمي على النظام الذي أصلح هذا. عملت مع مهندس معماري رئيسي لتجربة المستخدم ومهندس واجهة أمامية. بنينا تسلسلًا هرميًا للمكوّنات وفق منهجية Atomic Design. يعتمد هذا التسلسل على نظام رموز تصميم (design tokens) بثلاث طبقات باستخدام OKLCH. كما بنينا روابط Code Connect. ساعدت هذه الروابط نموذج LLM على كتابة الكود أيضًا، بدقة وصلت إلى ٨٠٪ في نموذج أولي عملي.",
      "case.bds.summary.problem": "<strong>المشكلة.</strong> كانت خمسة فرق في منصة Hub تبني نفس الزر بخمس طرق مختلفة، دون رابط بين Figma والكود سوى لقطات الشاشة.",
      "case.bds.summary.role": "<strong>الدور.</strong> قدت العمل التصميمي، من البنية المعمارية الأساسية إلى التوثيق، إلى جانب مهندس معماري رئيسي لتجربة المستخدم (صاحب القرار النهائي) ومهندس واجهة أمامية (المرحلة الثانية وCode Connect).",
      "case.bds.summary.impact": "<strong>الأثر.</strong> طابقت الأداة ٨٠٪ من المكوّنات بدقة في نموذج أولي لتحويل التصميم إلى كود. اكتملت المرحلة الأولى، والمرحلة الثانية قيد التطوير، ولم تُطلق بعد في الإنتاج.",
      "case.bds.summary.stack": "<strong>الأدوات.</strong> Figma · رموز OKLCH · Atomic Design · Code Connect · React · Tailwind v4",
      ...
      "case.hub.summary.problem": "<strong>المشكلة.</strong> استلزم تغيير نموذج بيانات Power Pages فتح Bootstrap ٥، مما فرض إعادة بناء كاملة للواجهة الأمامية، وكشف عن ٣٥ مخالفة حرجة وخطيرة في WCAG، وتدفقات Azure AD B2C غير متطابقة، وثلاثة مؤشرات تحميل غير منسّقة.",
      "case.hub.summary.role": "<strong>الدور.</strong> صممت ترقية النظام البصري، وطورت الواجهة الأمامية لإعادة بناء Bootstrap، وتدفقات Azure AD B2C الإحدى عشر، وتوحيد مؤشر التحميل.",
      "case.hub.summary.impact": "<strong>الأثر.</strong> عولجت ٣٥ مخالفة WCAG لتصل إلى ٠. دُمج ١٤ من نحو ٢٠ ملف CSS على مستوى الصفحة في ملف تصميم واحد قائم على الرموز. أُطلقت جميع الأجزاء الخمسة في الإنتاج.",
      "case.hub.summary.stack": "<strong>الأدوات.</strong> Bootstrap من الإصدار ٣ إلى ٥ · Power Pages · Dynamics 365 · Azure AD B2C · Deque Axe",
      ...
      "case.speery.summary.problem": "<strong>المشكلة.</strong> كان النموذج الأولي الذي بناه الذكاء الاصطناعي مكتملًا وظيفيًا لكنه فارغ من حيث البنية: الألوان لا تحمل معنى ثابتًا، لا يوجد تسلسل هرمي، ولا شيء يوحي بالثقة لمشترٍ مؤسسي.",
      "case.speery.summary.role": "<strong>الدور.</strong> عملت بمفردي، من التصميم حتى التسليم. استخدمت Figma فقط، وتولى فريق هندسة العميل البناء الفعلي.",
      "case.speery.summary.impact": "<strong>الأثر.</strong> إعادة بناء كاملة لهيكلة المعلومات، ونظام وسم ثلاثي المحاور، ونمط تنقل قابل لإعادة الاستخدام، أُنجز خلال أسبوعين من التصميم حتى التسليم.",
      "case.speery.summary.stack": "<strong>الأدوات.</strong> Figma · نماذج أولية بـ Smart Animate · نظام ألوان ووسم دلالي",
      ...
      "case.speery.readtwice.p2": "طلب المختصر نظامًا محددًا بما يكفي ليقرأه ذكاء اصطناعي لاحقًا، لا شخص فقط. هذه نفس الفكرة وراء بنية رموز Blueprint، المبنية لكل من المطوّر البشري ونموذج LLM منذ اليوم الأول. راجع دراسة حالة <a href=\"blueprint-design-system.html\">Blueprint Design System</a>.",
